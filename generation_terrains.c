...
void generation_aleatoire(Terrain *t, int l, int h, float dObst) {
  /* À compléter */
  //Les deux boucles sont la pour traiter tout les elements du terrain lignes et colonnes
  for (int j=0; j<h; j++){
    for (int i=0; i<l; i++){
        //initialisation nombre aleatoire entre 1 et 100
        int randint = (rand()%100)+1;
        //on traduit dobst en pourcentage, la condition combiné au randint dessus, correspond à une probabilitée de placer un obstacle
        if (randint < (dObst*100)){
          if (randint % 2 == 1){
            t->tab[i][j]=EAU;
          }
          else {
