...
/*check si une case de coordonnee x,y est libre et renvoie 1 si elle l'est sinon 0*/
...
/*ecrit le terrain dans un fichier fourni en argument*/
void ecrire_terrain(FILE *f, Terrain *T, int x, int y) {
  //init
  int i, j;
  char c;

  //ecrit la largeur et la heuteur du terrain dans le fichier
  fprintf(f, "%d\n", T->largeur);
  fprintf(f, "%d\n", T->hauteur);
  //ecrit le terrain dans le fichier
  for (j = 0; j < T->hauteur; j++) {
    for (i = 0; i < T->largeur; i++) {
      switch (T->tab[i][j]) {
      case LIBRE:
